...
def two_pass(image, mode = 'default'):
    ...
    for i in range(h): #Percorre a imagem
        for j in range(w):
            if im_label[i,j]!=0: #Se encontrar 1 label
                for k in range(equiv.shape[0]-1,-1,-2): #Percorre o vetor de equivalências de trás pra frente
                    if im_label[i,j] == equiv[k]: #Se o label atual estiver nas equivalências
                        im_label[i,j] = equiv[k-1] #Troca pelo seu valor anterior
...
numero_objetos, I1 = two_pass(image,'reform')
